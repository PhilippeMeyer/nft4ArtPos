...
import os from 'os';
const webSite = 'http://192.168.1.5:8999';
...
let metas = []; // list of the Nfts loaded from the smart contract
...
function loadHandler() {
    ...
    if (registeredPoS === null)
        registeredPoS = db.addCollection('registeredPoS');
...
waitFor(() => databaseInitialized == true).then(() => server.listen(process.env.PORT || 8999, () => {
...
//
app.post('/apiV1/auth/registerPoS', function (req, res) {
});
//
// /apiV1/auth/sigin
...
//
function registerPoS(device, pass, res) {
    var manager;
    if (!device.authorized)
        device.authorized = false;
    manager = typeof pass === 'undefined' ? false : true;
    var pos = registeredPoS.findOne({ deviceId: device.deviceId });
    if (pos == null) {
...
app.get('/tokens', verifyToken, function (req, res) {
...
//	- the price
//
app.put('/apiV1/price/update', verifyTokenManager, function (req, res) {
    ...
    var token = tokens.findOne({ id: req.query.tokenId });
    if (token == null) {
...
//
app.put('/apiV1/price/updates', verifyTokenManager, function (req, res) {
...
// This end point sends back the registered PoS
//
app.put('/apiV1/auth/authorizePoS', verifyTokenManager, function (req, res) {
...
app.get('/apiV1/generateWallets', verifyTokenManager, async function (req, res) {
    let nbWallets = 10;
    if (req.query.nbWallets !== undefined)
        nbWallets = parseInt(req.query.nbWallets);
...
function sendLock(id, isLocked) {
    sendMessage(JSON.stringify(new LockMessage(id, isLocked)));
}
function sendPrice(id, price) {
...
async function init() {
    ...
    for (i = 0; i < events.length; i++) {
        ...
        if (errTimeout == 2)
            break; // If we face a timeout we retry twice
        try {
            ...
            if (typeof err.response != 'undefined') { // We have receieved a proper response but containing an error	
                ...
                if (err.response.status == 404)
                    break; // 404 = not found, we stop as the information is not available
            }
            else
